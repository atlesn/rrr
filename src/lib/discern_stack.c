...
static int __rrr_discern_stack_storage_push (
	...
	if (data >= target->data && data < target->data + target->capacity) {
		// Copy from and to same storage, just return
		// the position of the existing data.
		rrr_length_from_ptr_sub_err(position, data, target->data);
		goto out;
	}
...
static void __rrr_discern_stack_list_clear (struct rrr_discern_stack_list *discern_stack) {
	RRR_FREE_IF_NOT_NULL(discern_stack->elements);
	memset(discern_stack, '\0', sizeof(*discern_stack));
}
...
static void __rrr_discern_stack_destroy (
	...
	__rrr_discern_stack_list_clear(&discern_stack->stack);
...
static int __rrr_discern_stack_list_push (
	...
	if (discern_stack->wpos == discern_stack->size) {
		const rrr_length size_new = rrr_length_add_bug_const(discern_stack->size, 8);
		struct rrr_discern_stack_element *elements;
		if ((elements = rrr_reallocate (
				discern_stack->elements,
				discern_stack->size * sizeof(*elements),
				size_new * sizeof(*elements)
		)) == NULL) {
			RRR_MSG_0("Could not allocate memory in %s\n", __func__);
			ret = 1;
			goto out;
		}
		discern_stack->elements = elements;
		discern_stack->size = size_new;
	}
...
static int __rrr_discern_stack_add_from (
	...
	for (rrr_length i = 0; i < source->list.wpos; i++) {
		...
		if ((ret = __rrr_discern_stack_list_push (
				...
				node->type,
				node->op,
				node->data_size > 0 ? source->storage.data + node->data_pos : NULL,
				node->data_size,
				node->value
		)) != 0) {
			goto out;
...
static int __rrr_discern_stack_execute_resolve_and_push (
	...
	if ((ret = resolve_cb (&result, storage->data + node->data_pos, callback_arg)) != 0) {
		goto out;
	}
	...
	*fault = RRR_DISCERN_STACK_FAULT_OK;
	...
	if (eval_two) {
		...
		if (b->type != RRR_DISCERN_STACK_E_BOOL)
			goto out_fail_type;
		result = eval_two(a->value, b->value);
		__rrr_discern_stack_list_pop(stack);
		__rrr_discern_stack_list_pop(stack);
	}
	else {
		result = eval_one(a->value);
		__rrr_discern_stack_list_pop(stack);
	}
...
static int __rrr_discern_stack_execute_op_apply (
	...
	*fault = RRR_DISCERN_STACK_FAULT_OK;
	...
	if ((ret = apply_cb(b->value, storage->data + a->data_pos, callback_arg)) != 0) {
		*fault = RRR_DISCERN_STACK_FAULT_CRITICAL;
		goto out;
	}
...
#define OP_FUNC_NAME(name) \
  RRR_PASTE(__rrr_discern_stack_execute_step_op_,name)

#define OP_FUNC(name,code)                                                                                                     \
  static int OP_FUNC_NAME(name)(OP_FUNC_ARGS) { int ret = 0; code; out: return ret; }

typedef int (*op_func)(OP_FUNC_ARGS);
...
OP_FUNC(PUSH,
	switch (args->node->type) {
		...
		if ((ret = __rrr_discern_stack_execute_resolve_and_push (
				...
				args->storage,
				args->node,
				args->resolve_topic_filter_cb,
				args->resolve_cb_arg
		)) != 0) {
			*args->fault = RRR_DISCERN_STACK_FAULT_CRITICAL;
			goto out;
		} break;
		...
		if ((ret = __rrr_discern_stack_list_push (
				...
				args->node->type,
				args->node->op,
				args->storage->data + args->node->data_pos,
				args->node->data_size,
				args->node->value
		)) != 0) {
			*args->fault = RRR_DISCERN_STACK_FAULT_CRITICAL;
...
OP_FUNC(AND,
	args->stack->elements[args->stack->wpos - 1].value =
		args->stack->elements[args->stack->wpos - 1].value &&
		args->stack->elements[args->stack->wpos - 2].value;
	args->stack->wpos--;
	goto out;
)
OP_FUNC(OR,
...
OP_FUNC(NOT,
	args->stack->elements[args->stack->wpos - 1].value = !args->stack->elements[args->stack->wpos - 1].value;
	goto out;
)
OP_FUNC(POP,
	args->stack->wpos--;
...
OP_FUNC(BAIL,
	if (args->stack->elements[args->stack->wpos - 1].value) {
...
static int __rrr_discern_stack_execute (
	...
	*fault = RRR_DISCERN_STACK_FAULT_OK;
	...
	static const op_func op_jump_table[] = {
		OP_FUNC_NAME(PUSH),
		OP_FUNC_NAME(AND),
		OP_FUNC_NAME(OR),
		OP_FUNC_NAME(APPLY),
	...
	for (rrr_length i = 0; i < discern_stack->list.wpos; i++) {
		args.node = &discern_stack->list.elements[i];
		if ((ret = op_jump_table[args.node->op](&args)) != 0) {
...
int rrr_discern_stack_collection_iterate_destination_names (
	...
	RRR_LL_ITERATE_BEGIN(collection, const struct rrr_discern_stack);
		...
		for (rrr_length i = 0; i < discern_stack->list.wpos; i++) {
			...
			if ((ret = callback(discern_stack->name, node->storage.data + e->data_pos, callback_arg)) != 0) {
				goto out;
			}
		}
	RRR_LL_ITERATE_END();
