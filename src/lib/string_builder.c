...
int rrr_string_builder_append (struct rrr_string_builder *string_builder, const char *str) {
	...
	if (string_builder->wpos + length + 1 > string_builder->size) {
		ssize_t new_size = length + 1 + string_builder->size + 1024;
		char *new_buf = realloc(string_builder->buf, new_size);
		if (new_buf == NULL) {
			VL_MSG_ERR("Could not allocate memory in rrr_string_builder_append\n");
			return 1;
		}
