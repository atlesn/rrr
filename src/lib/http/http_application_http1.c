...
static int __rrr_application_http1_websocket_make_accept_string (
		char **accept_str,
	...
	if (rrr_asprintf(&accept_str_tmp, "%s%s", sec_websocket_key, RRR_HTTP_WEBSOCKET_GUID) <= 0) {
		RRR_MSG_0("Failed to concatenate accept-string in __rrr_application_http1_make_websocket_accept_string\n");
	...
	if (!rrr_SHA1Result(&sha1_ctx) || sha1_ctx.Corrupted != 0 || sha1_ctx.Computed != 1) {
		RRR_MSG_0("Computation of SHA1 failed in __rrr_application_http1_websocket_make_accept_string (Corrupt: %i - Computed: %i)\n",
	...
	if ((accept_base64_tmp = (char *) rrr_base64_encode (
		...
		RRR_MSG_0("Base64 encoding failed in __rrr_application_http1_websocket_make_accept_string\n");
		ret = RRR_HTTP_SOFT_ERROR;
		goto out;
	}
...
static int __rrr_application_http1_websocket_response_check_headers (
		struct rrr_application_http1 *session
) {
	int ret = 0;
	...
	if ((ret = rrr_websocket_state_get_key_base64(&sec_websocket_key_tmp, &session->ws_state)) != 0) {
		RRR_MSG_0("Failed to get key from WebSocket state in __rrr_application_http1_request_receive_try_websocket\n");
		goto out;
	}

	if ((ret = __rrr_application_http1_websocket_make_accept_string(&accept_str_tmp, sec_websocket_key_tmp)) != 0) {
...
static void __rrr_application_http1_free_dbl_ptr (void *ptr) {
...
int rrr_application_http1_transport_ctx_request_send (
		struct rrr_net_transport_handle *handle,
		struct rrr_http_application *application,
		const char *host
) {
	return application->constants->request_send(application, handle, host);
}

int rrr_application_http1_transport_ctx_raw_request_send (
		struct rrr_net_transport_handle *handle,
		const char *raw_request_data,
		size_t raw_request_size
) {
	if (raw_request_size == 0) {
		RRR_BUG("BUG: Received 0 size in rrr_application_http1_transport_ctx_raw_request_send\n");
	}
...
struct rrr_application_http1_receive_data {
	...
	int (*websocket_callback)(rrr_application_http1_WEBSOCKET_HANDSHAKE_CALLBACK_ARGS);
	void *websocket_callback_arg;
	int (*callback)(rrr_application_http1_RECEIVE_CALLBACK_ARGS);
	void *callback_arg;
	int (*raw_callback)(rrr_application_http1_RAW_RECEIVE_CALLBACK_ARGS);
...
static int __rrr_application_http1_response_receive_callback (
	...
	struct rrr_application_http1 *session = receive_data->handle->application_private_ptr;
	...
	if (session->response_part->response_code == RRR_HTTP_RESPONSE_CODE_SWITCHING_PROTOCOLS) {
		...
		if (field_upgrade_websocket != NULL) {
			...
			if ((ret = __rrr_application_http1_websocket_response_check_headers(session)) != 0) {
				goto out;
			}
	...
	out:
	__rrr_application_http1_destroy_part_if_not_null(&session->response_part);
	__rrr_application_http1_destroy_part_if_not_null(&session->request_part);
	RRR_FREE_IF_NOT_NULL(orig_http2_settings_tmp);
	return ret;
}

struct rrr_application_http1_send_header_field_callback_data {
	struct rrr_net_transport_handle *handle;
};

static int __rrr_application_http1_send_header_field_callback (struct rrr_http_header_field *field, void *arg) {
	struct rrr_application_http1_send_header_field_callback_data *callback_data = arg;
	...
	if (!rrr_nullsafe_str_isset(field->name) || !rrr_nullsafe_str_isset(field->value)) {
		RRR_BUG("BUG: Name or value was NULL in __rrr_application_http1_send_header_field_callback\n");
	}
	if (RRR_LL_COUNT(&field->fields) > 0) {
		RRR_BUG("BUG: Subvalues were present in __rrr_application_http1_send_header_field_callback, this is not supported\n");
	}

	pthread_cleanup_push(__rrr_application_http1_free_dbl_ptr, &send_data);
	...
	if ((send_data_length = rrr_asprintf(&send_data, "%s: %s\r\n", name, value)) <= 0) {
		RRR_MSG_0("Could not allocate memory for header line in __rrr_application_http1_send_header_field_callback\n");
		ret = 1;
		goto out;
	}
	...
	if ((ret = rrr_net_transport_ctx_send_blocking(callback_data->handle, send_data, send_data_length)) != 0) {
		RRR_DBG_1("Error: Send failed in __rrr_application_http1_send_header_field_callback\n");
		goto out;
	}
...
static int __rrr_application_http1_transport_ctx_send_response (
		struct rrr_net_transport_handle *handle
) {
	struct rrr_application_http1 *session = handle->application_private_ptr;
	struct rrr_http_part *response_part = session->response_part;

	int ret = 0;

	if (response_part == NULL) {
		RRR_BUG("BUG: Response part was NULL in rrr_application_http1_send_response\n");
	}
	...
	if (response_part->response_code == 0) {
		RRR_BUG("BUG: Response code was not set in rrr_application_http1_send_response\n");
	}
	...
	switch (response_part->response_code) {
		...
		case RRR_HTTP_RESPONSE_CODE_VERSION_NOT_SUPPORTED:
			...
			break;
		default:
			RRR_BUG("BUG: Response code %i not implemented in rrr_application_http1_send_response\n",
	...
	struct rrr_application_http1_send_header_field_callback_data callback_data = {
			handle
	};

	if ((ret = rrr_http_part_header_fields_iterate(response_part, __rrr_application_http1_send_header_field_callback, &callback_data)) != 0) {
		goto out_err;
	}
	...
	goto out;
	out_err:
		RRR_MSG_0("Error while sending headers for HTTP client %i in rrr_application_http1_transport_ctx_send_response\n",
				handle->handle);
	out:
		return ret;
}
...
static int __rrr_application_http1_request_upgrade_try_websocket (
	...
	*do_websocket = 1;
	...
	if (__rrr_application_http1_websocket_request_check_version(session) != 0) {
		goto out_bad_request;
	}
	...
	if ((ret = __rrr_application_http1_websocket_make_accept_string(&accept_base64_tmp, sec_websocket_key_str)) != 0) {
		RRR_MSG_0("Failed to make accept-string in __rrr_application_http1_request_receive_try_websocket\n");
		goto out;
	}
...
//	const struct rrr_http_header_field *content_type = rrr_http_part_get_header_field(part, "content-type");
	...
	if ((ret = __rrr_application_http1_prepare_part(&session->response_part)) != 0) {
		RRR_MSG_0("Failed to prepare response part in __rrr_application_http1_request_receive_callback\n");
		goto out;
	}
	...
	if ((ret = __rrr_application_http1_transport_ctx_send_response(receive_data->handle)) != RRR_HTTP_OK) {
		goto out;
	}
...
//		printf("pos: %02x\n", *pos);
	...
	if (*part_to_use == NULL) {
		if (rrr_http_part_new(part_to_use) != 0) {
			RRR_MSG_0("Could not create new part in __rrr_application_http1_receive_get_target_size\n");
...
int rrr_application_http1_transport_ctx_receive (
		struct rrr_net_transport_handle *handle,
	...
	//	if ((ret = __rrr_application_http1_prepare_parts(callback_data.session)) != 0) {
	//		goto out;
	//	}
...
struct rrr_application_http1_websocket_frame_callback_data {
	struct rrr_application_http1 *session;
	rrr_http_unique_id unique_id;
	int (*callback)(rrr_application_http1_WEBSOCKET_FRAME_CALLBACK_ARGS);
	void *callback_arg;
};

static int __rrr_application_http1_websocket_frame_callback (
		RRR_WEBSOCKET_FRAME_CALLBACK_ARGS
) {
	struct rrr_application_http1_websocket_frame_callback_data *callback_data = arg;
...
static int __rrr_application_http1_websocket_get_responses (
		struct rrr_websocket_state *ws_state,
		int (*get_response_callback)(rrr_application_http1_WEBSOCKET_GET_RESPONSE_CALLBACK_ARGS),
...
int rrr_application_http1_transport_ctx_websocket_tick (
		struct rrr_net_transport_handle *handle,
		ssize_t read_max_size,
		rrr_http_unique_id unique_id,
		int ping_interval_s,
		...
		int (*get_response_callback)(rrr_application_http1_WEBSOCKET_GET_RESPONSE_CALLBACK_ARGS),
		void *get_response_callback_arg,
		int (*frame_callback)(rrr_application_http1_WEBSOCKET_FRAME_CALLBACK_ARGS),
	...
	struct rrr_application_http1_websocket_frame_callback_data callback_data = {
			session,
			unique_id,
			frame_callback,
			frame_callback_arg
	...
	if ((ret = __rrr_application_http1_websocket_get_responses (
			&session->ws_state,
	...
	if ((ret = (rrr_websocket_transport_ctx_read_frames (
			...
			65535,
			read_max_size,
			__rrr_application_http1_websocket_frame_callback,
			&callback_data
...
static int __rrr_application_http1_multipart_form_data_body_send_wrap_chunk (
		struct rrr_net_transport_handle *handle,
		const void *data,
		ssize_t size
) {
	if (size < 0) {
		RRR_BUG("Size was < 0 in __rrr_application_http1_multipart_form_data_body_send_wrap_chunk\n");
	}
	if (size == 0) {
		return RRR_HTTP_OK;
	}
...
static int __rrr_application_http1_multipart_field_send (
		struct rrr_net_transport_handle *handle,
		const char *boundary,
	...
	if (rrr_nullsafe_str_isset(node->name)) {
		if ((name_buf = rrr_http_util_quote_header_value_nullsafe(node->name, '"', '"')) == NULL) {
			RRR_MSG_0("Could not quote field name_buf in __rrr_application_http1_multipart_field_send\n");
			ret = 1;
			goto out;
		}

		if ((ret = rrr_asprintf (&name_buf_full, "; name=%s", name_buf)) <= 0) {
			RRR_MSG_0("Could not create name_buf_full in __rrr_application_http1_multipart_field_send return was %i\n", ret);
			ret = 1;
			goto out;
		}
	}
	...
	if (rrr_nullsafe_str_isset(node->content_type)) {
		...
		if ((ret = rrr_asprintf (&content_type_buf, "Content-Type: %s\r\n", value)) <= 0) {
			RRR_MSG_0("Could not create content_type_buf in __rrr_application_http1_multipart_field_send return was %i\n", ret);
			ret = 1;
			goto out;
		}
	}
	...
	if ((ret = rrr_asprintf (
		...
		RRR_MSG_0("Could not create content type string and body  in __rrr_application_http1_multipart_field_send return was %i\n", ret);
		ret = 1;
		goto out;
	}

	if ((ret = __rrr_application_http1_multipart_form_data_body_send_wrap_chunk(handle, body_buf, strlen(body_buf))) != 0) {
		RRR_MSG_0("Could not send form part of HTTP request in __rrr_application_http1_multipart_field_send A\n");
		goto out;
	}

	if (rrr_nullsafe_str_isset(node->value)) {
		if ((ret = __rrr_application_http1_multipart_form_data_body_send_wrap_chunk(
				handle,
				node->value->str,
				node->value->len
		)) != 0) {
			RRR_MSG_0("Could not send form part of HTTP request in __rrr_application_http1_multipart_field_send B\n");
			goto out;
		}
	}
...
static int __rrr_application_http1_multipart_form_data_body_send (
		struct rrr_net_transport_handle *handle
) {
	struct rrr_application_http1 *session = handle->application_private_ptr;
	...
	pthread_cleanup_push(__rrr_application_http1_free_dbl_ptr, &body_buf);
	pthread_cleanup_push(__rrr_application_http1_free_dbl_ptr, &boundary_buf);

	// RFC7578

	if ((ret = rrr_asprintf (&boundary_buf, "RRR%u", (unsigned int) rrr_rand())) < 0) {
		RRR_MSG_0("Could not create boundary_buf string in __rrr_application_http1_multipart_form_data_body_send return was %i\n", ret);
		ret = 1;
		goto out;
	}

	{
		...
		if ((ret = rrr_asprintf (
			...
			RRR_MSG_0("Could not create content type string in __rrr_application_http1_multipart_form_data_body_send return was %i\n", ret);
			ret = 1;
			goto out;
		}

		if ((ret = rrr_net_transport_ctx_send_blocking(handle, body_buf, strlen(body_buf))) != 0) {
			RRR_DBG_1("Could not send first part of HTTP request in __rrr_application_http1_multipart_form_data_body_send\n");
			goto out;
		}
	}
	...
	RRR_LL_ITERATE_BEGIN(&session->request_part->fields, struct rrr_http_field);
		if ((ret = __rrr_application_http1_multipart_field_send(handle, boundary_buf, node, is_first)) != 0) {
			goto out;
		}
		is_first = 0;
	RRR_LL_ITERATE_END();

	{
		...
		if ((ret = rrr_asprintf (
			...
			RRR_MSG_0("Could not create last boundary in __rrr_application_http1_multipart_form_data_body_send return was %i\n", ret);
			ret = 1;
			goto out;
		}

		if ((ret = __rrr_application_http1_multipart_form_data_body_send_wrap_chunk(handle, body_buf, strlen(body_buf))) != 0) {
			RRR_MSG_0("Could not send last part of HTTP request in __rrr_application_http1_multipart_form_data_body_send\n");
			goto out;
		}
	}

	if ((ret = rrr_net_transport_ctx_send_blocking(handle, "0\r\n\r\n", 5)) != 0) {
		RRR_DBG_1("Could not send terminating chunk of HTTP request in __rrr_application_http1_multipart_form_data_body_send\n");
		goto out;
	}
...
static int __rrr_application_http1_post_x_www_form_body_send (
		struct rrr_net_transport_handle *handle,
		int no_urlencoding
) {
	struct rrr_application_http1 *session = handle->application_private_ptr;
	...
	pthread_cleanup_push(__rrr_application_http1_free_dbl_ptr, &body_buf);
	pthread_cleanup_push(__rrr_application_http1_free_dbl_ptr, &header_buf);
	...
	if (body_buf == NULL) {
		RRR_MSG_0("Could not create body in __rrr_application_http1_send_post_urlencoded_body\n");
		ret = 1;
		goto out;
	}
	...
	if ((ret = rrr_asprintf (
		...
		RRR_MSG_0("Could not create content type string in __rrr_application_http1_send_get_body return was %i\n", ret);
		ret = 1;
		goto out;
	}

	if ((ret = rrr_net_transport_ctx_send_blocking (handle, header_buf, strlen(header_buf))) != 0) {
		RRR_DBG_1("Could not send GET body header in __rrr_application_http1_send_get_body\n");
		goto out;
	}

	if ((ret = rrr_net_transport_ctx_send_blocking (handle, body_buf, body_size)) != 0) {
		RRR_DBG_1("Could not send GET body in __rrr_application_http1_send_get_body\n");
		goto out;
	}
...
static int __rrr_application_http1_request_send_make_headers_callback (
...
static int __rrr_http_application_http1_request_send (
	...
	pthread_cleanup_push(__rrr_application_http1_free_dbl_ptr, &request_buf);
	pthread_cleanup_push(__rrr_application_http1_free_dbl_ptr, &host_buf);
	pthread_cleanup_push(__rrr_application_http1_free_dbl_ptr, &user_agent_buf);
	pthread_cleanup_push(__rrr_application_http1_free_dbl_ptr, &uri_tmp);
	pthread_cleanup_push(__rrr_application_http1_free_dbl_ptr, &extra_uri_tmp);
	pthread_cleanup_push(__rrr_application_http1_free_dbl_ptr, &websocket_key_tmp);
	pthread_cleanup_push(__rrr_application_http1_free_dbl_ptr, &http2_upgrade_settings_tmp);
	...
	if (rrr_string_builder_new(&header_builder) != 0) {
		RRR_MSG_0("Failed to create string builder in __rrr_application_http1_request_send\n");
		ret = 1;
		goto out_final;
	}
	...
	if (host_buf == NULL) {
		RRR_MSG_0("Invalid host '%s' in __rrr_application_http1_send_request\n", callback_data->host);
		ret = 1;
		goto out;
	}
	...
	if (user_agent_buf == NULL) {
		RRR_MSG_0("Invalid user agent '%s' in __rrr_application_http1_send_request\n", session->user_agent);
		ret = 1;
		goto out;
	}
	...
	if (session->method == RRR_HTTP_METHOD_GET && RRR_LL_COUNT(&session->request_part->fields) > 0) {
		...
		RRR_TYPES_BUG_IF_LENGTH_EXCEEDED(uri_orig_len,"rrr_application_http1_send_request");

		if ((uri_tmp = malloc(uri_orig_len + extra_uri_size + 1 + 1)) == NULL) { // + separator + 0
			RRR_MSG_0("Could not allocate memory for new URI in __rrr_application_http1_request_send\n");
			ret = 1;
			goto out;
		}
	...
	if ((ret = rrr_asprintf (
		...
		RRR_MSG_0("Error while making request string in rrr_application_http1_send_request return was %i\n", ret);
		ret = 1;
		goto out;
	}

	if ((ret = rrr_net_transport_ctx_send_blocking (handle, request_buf, strlen(request_buf))) != 0) {
		RRR_DBG_1("Could not send first part of HTTP request header in __rrr_application_http1_send_request\n");
		goto out;
	}
	...
	if (rrr_http_part_header_fields_iterate (
			session->request_part,
			__rrr_application_http1_request_send_make_headers_callback,
			header_builder
	) != 0) {
		RRR_MSG_0("Failed to make header fields in __rrr_application_http1_send_request\n");
		ret = 1;
		goto out;
	}
	...
	if (header_builder_length > 0) {
		...
		if ((ret = rrr_net_transport_ctx_send_blocking (handle, request_buf, header_builder_length)) != 0) {
			RRR_MSG_0("Could not send second part of HTTP request header in __rrr_application_http1_send_request\n");
			goto out;
		}
	}
	...
	if (session->method != RRR_HTTP_METHOD_GET && RRR_LL_COUNT(&session->request_part->fields) > 0) {
		if (session->method == RRR_HTTP_METHOD_POST_MULTIPART_FORM_DATA) {
			if ((ret = __rrr_application_http1_multipart_form_data_body_send (handle)) != 0) {
				RRR_MSG_0("Could not send POST multipart body in __rrr_application_http1_send_request\n");
				goto out;
			}
		}
		else if (session->method == RRR_HTTP_METHOD_POST_URLENCODED) {
			if ((ret = __rrr_application_http1_post_x_www_form_body_send (handle, 0)) != 0) {
				RRR_MSG_0("Could not send POST urlencoded body in __rrr_application_http1_send_request\n");
				goto out;
			}
		}
			...
			if ((ret = __rrr_application_http1_post_x_www_form_body_send (handle, 1)) != 0) {
				RRR_MSG_0("Could not send POST urlencoded body in __rrr_application_http1_send_request\n");
				goto out;
			}
		}
		...
		RRR_MSG_0("Could not send last \\r\\n in __rrr_application_http1_send_request\n");
		goto out;
	}
