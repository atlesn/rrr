...
static int __rrr_array_tree_interpret_identifier_and_size_tag (
	...
	rrr_length length = rrr_length_from_ptr_sub_bug(*start,  tag_begin);
	if (length == 0) {
	...
	*bytes_parsed_return = 0;
	...
	if (type->max_length > 0) {
		...
		if (*start == '{') {
			...
			rrr_length_add_bug (&parsed_bytes, rrr_length_from_ptr_sub_bug (integer_end, start));
			start = integer_end;
...
static int __rrr_array_tree_interpret_node (
	...
	*target = NULL;
	...
	while (!RRR_PARSE_CHECK_EOF(pos)) {
		...
		rrr_length length = end - start + 1; // +1 here is not for the \0
...
int __rrr_array_tree_interpret_rewind (
	...
	char *endptr;
	rrr_length count = rrr_length_from_size_t_bug(strtoul(tmp, &endptr, 10));
