...
struct rrr_net_transport_methods {
	...
	int (*close)(struct rrr_net_transport *transport, int handle);
	int (*read_message)(
...
static inline void rrr_net_transport_destroy (struct rrr_net_transport *transport) {
...
static inline int rrr_net_transport_close (
		struct rrr_net_transport *transport,
		int handle
) {
	return transport->methods->close(transport, handle);
}
