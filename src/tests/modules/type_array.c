...
#define TEST_POLL_CALLBACK_SIGNATURE \
	struct rrr_msg_holder *entry, struct rrr_test_data *data

int test_anything_callback (TEST_POLL_CALLBACK_SIGNATURE) {
	struct rrr_msg_msg *message = (struct rrr_msg_msg *) entry->message;
	const struct rrr_map *array_check_values = data->array_check_values;
...
struct test_poll_callback_intermediate_data {
...
static void __test_event_poll_cb (
	...
	struct rrr_test_data *data = arg;
	struct rrr_instance_runtime_data *thread_data = data->thread_data;
	...
	TEST_MSG("Test result polling try: %i of %i\n",
			data->loop_i, RRR_TEST_TYPE_ARRAY_LOOP_COUNT);

	struct test_poll_callback_intermediate_data data_intermediate = {
		data->callback,
		data->callback_arg
	};
...
//		rrr_array_dump(&array_tmp);
		...
		if ((value = rrr_array_value_get_by_tag(&array_tmp, "measurement")) != NULL) {
			// Copies of the original four point measurements should arrive first. Expect
			// that result is initialized to 1 prior to test starting.
			(*data->result)++;
		}
		else {
...
int test_averager (
	...
	data->callback = test_averager_callback;
	data->callback_arg = data;

	return __test_init_poll_loop (data);
}
...
int test_type_array_callback (TEST_POLL_CALLBACK_SIGNATURE) {
	...
	const struct rrr_test_function_data *config = data->config;
...
int test_array (
	...
	data->callback = test_type_array_callback;
	data->callback_arg = data;
...
int test_anything (
	...
	data->callback = test_anything_callback;
	data->callback_arg = data;
...
int test_type_array_mysql (
	...
	RRR_LL_ITERATE_BEGIN(INSTANCE_D_INSTANCES(data->thread_data), struct rrr_instance);
	...
	data->callback = test_type_array_mysql_and_network_callback;
	data->callback_arg = mysql_data;
	data->cleanup = test_type_array_mysql_data_cleanup;
	data->cleanup_arg = mysql_data;
