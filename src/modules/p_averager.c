...
struct averager_data {
	...
	struct rrr_message_holder_collection input_list;
	struct rrr_message_holder_collection output_list;
...
int averager_poll_callback(RRR_MODULE_POLL_CALLBACK_SIGNATURE) {
	...
	struct rrr_message_holder *dup_entry = NULL;
	...
	if (MSG_IS_MSG(message) && MSG_IS_ARRAY(message)) {
		...
		if (averager_data->preserve_point_measurements == 1) {
			...
			if (rrr_message_holder_util_clone_no_locking(&dup_entry, entry) != 0) {
			...
			rrr_message_holder_lock(dup_entry);
			...
			// Due to linked list
			rrr_message_holder_incref_while_locked(dup_entry);

			rrr_message_holder_unlock(dup_entry);
	...
	out:
		if (dup_entry != NULL) {
			rrr_message_holder_decref(dup_entry);
		}
...
void averager_maintain_buffer(struct averager_data *data) {
	...
	RRR_LL_ITERATE_BEGIN(&data->input_list, struct rrr_message_holder);
	...
	RRR_LL_ITERATE_END_CHECK_DESTROY(&data->input_list, 0; rrr_message_holder_decref_while_locked_and_unlock(node));
}
...
int averager_process_message (
		...
		struct rrr_message_holder *entry_locked
) {
...
int averager_spawn_message_callback (struct rrr_message_holder *new_entry, void *arg) {
...
void averager_data_cleanup(void *arg) {
	...
	rrr_message_holder_collection_clear (&data->input_list);
	rrr_message_holder_collection_clear (&data->output_list);
