...
static int modbus_poll_callback (RRR_MODULE_POLL_CALLBACK_SIGNATURE) {
	...
	if (data->message_ttl_us > 0 && !rrr_msg_msg_ttl_ok(message, data->message_ttl_us)) {
		RRR_MSG_0("Warning: Received message in modbus instance %s with expired TTL, limit is set to %" PRIrrrbl " seconds. Dropping message.\n",
				INSTANCE_D_NAME(thread_data), data->message_ttl_seconds);
		goto drop;
	}

	RRR_DBG_3("modbus instance %s received a message with timestamp %llu\n",
...
static int modbus_parse_config (struct modbus_data *data, struct rrr_instance_config_data *config) {
	...
	RRR_INSTANCE_CONFIG_PARSE_OPTIONAL_UNSIGNED("modbus_ttl_seconds", message_ttl_seconds, 0);

	if (data->message_ttl_seconds > UINT32_MAX) {
		RRR_MSG_0("modbus_ttl_seconds too large in instance %s, maximum is %u\n", config->name, UINT32_MAX);
		ret = 1;
		goto out;
	}
