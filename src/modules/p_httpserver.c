...
static int httpserver_async_response_get_and_process (
	...
	if (callback_data.entry != NULL) {
		...
		msg = callback_data.entry->message;

		RRR_DBG_3("httpserver instance %s got a response from senders with topic filter '%s' age %" PRIu64 "\n",
				INSTANCE_D_NAME(data->thread_data), topic_filter_use, rrr_time_get_64() - msg->timestamp);
...
static int httpserver_poll_callback (RRR_POLL_CALLBACK_SIGNATURE) {
